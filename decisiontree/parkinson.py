...
def runParkinson(isGini, minInfoGainMaxGiniIndex):
    ...
    plt.title('correlation between accuracy and number of trees (parkinson)')
    fig.savefig("accuracyParkinson.png")
    fig1 = plt.figure()
    plt.plot(ntreeValues, precision)
    plt.ylabel('precision')
    plt.xlabel('number of trees')
    plt.title('correlation between precision and number of trees (parkinson)')
    fig1.savefig("precisionParkinson.png")
    fig2 = plt.figure()
    plt.plot(ntreeValues, recall)
    plt.ylabel('recall')
    plt.xlabel('number of trees')
    plt.title('correlation between recall and number of trees (parkinson)')
    fig2.savefig("recallParkinson.png")
    fig3 = plt.figure()
    plt.plot(ntreeValues, f1)
    plt.ylabel('f1')
    plt.xlabel('number of trees')
    plt.title('correlation between f1 score and number of trees (parkinson)')
    fig3.savefig("f1Parkinson.png")
    plt.show()
