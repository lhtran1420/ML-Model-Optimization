...
def runParkinsons(isGini, minInfoGainMaxGiniIndex):
    accuracy = []
    precision = []
    recall = []
    f1 = []
    ...
    for i in range(len(kval)):
        newAccuracy, newPrecision, newRecall, newF1 = parkinsons(dataset, kval[i])
    ...
    plt.title('correlation between accuracy and number of kval (parkinsons)')
    fig.savefig("figures/accuracyParkinsons.png")
    fig1 = plt.figure()
    plt.plot(kval, precision)
    plt.ylabel('precision')
    plt.xlabel('number of kval')
    plt.title('correlation between precision and number of kval (parkinsons)')
    fig1.savefig("figures/precisionParkinsons.png")
    fig2 = plt.figure()
    plt.plot(kval, recall)
    plt.ylabel('recall')
    plt.xlabel('number of kval')
    plt.title('correlation between recall and number of kval (parkinsons)')
    fig2.savefig("figures/recallParkinsons.png")
    fig3 = plt.figure()
    plt.plot(kval, f1)
    plt.ylabel('f1')
    plt.xlabel('number of kval')
    plt.title('correlation between f1 score and number of kval (parkinsons)')
    fig3.savefig("figures/f1Parkinsons.png")
    plt.show()
